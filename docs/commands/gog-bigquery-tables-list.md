...
```bash
gog bigquery (bq) tables list (ls) <dataset> [flags]
```

## Parent

- [gog bigquery tables](gog-bigquery-tables.md)
...
| `--fail-empty`<br>`--non-empty`<br>`--require-results` | `bool` |  | Exit with code 3 if no tables |
